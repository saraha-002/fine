...
const replacements = [
  // File name in links
  { from: /athiriver-escorts\.html/g, to: 'athiriver-escorts.html' },
  // Slug in URLs (e.g., /blog/athiriver-...)
  { from: /athiriver/g, to: 'athiriver' },
...
// Rename the actual file athiriver-escorts.html → athiriver-escorts.html if it exists
const oldFile = path.join(rootDir, 'athiriver-escorts.html');
const newFile = path.join(rootDir, 'athiriver-escorts.html');
if (fs.existsSync(oldFile)) {
  fs.renameSync(oldFile, newFile);
  console.log(`📄 Renamed file: athiriver-escorts.html → athiriver-escorts.html`);
} else {
  console.log(`ℹ️ File athiriver-escorts.html not found – skipping rename.`);
}
